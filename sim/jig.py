...
def jig(override: dict, supressed: bool, independent_variable: str) -> str:
    ...
    combinations = functions.sort_combinations(
        config,
        (
            list(functions.cartesian_product(override))
            if override
            else [{"settings": {"name": config["settings"]["name"][0]}}]
        ),
    )
    for run_number, combination in enumerate(combinations):
        job_progress.add_task(
            f"[green]{run_number}",
            total=combination["experimental_time"].total_seconds(),
        )
    ...
    with Live(
        Layout() if config["settings"]["display"] else progress_table,
        refresh_per_second=60,
        screen=False if config["settings"]["display"] else False,
    ) as live:
        if not supressed:
        ...
        runs = [
            model.run(
                settings.Config(
                    functions.add_time_num(combination, start_time, run_number)
                ),
                live,
                job_progress,
                overall_progress,
...
def stats(folder: str, independent_variable: str) -> bool:
    ...
    runs = None
    with open(
        f"results/{folder}/dictionaries/config.dictionary", "rb"
    ) as config_dictionary_file:
        config = pickle.load(config_dictionary_file)
    with open(
        f"results/{folder}/dictionaries/runs.dictionary", "rb"
    ) as runs_dictionary_file:
...
def old_display(folder: str) -> bool:
    ...
    if config["settings"]["save_type"][0] == enums.SaveType.DETAILED:
        ...
        if len(config["samples"]["samples"][0]) > 1:
            ...
            fig.update_traces(
                showlegend=True, error_y_color="red", marker=dict(color=colors)
            )
            fig.update_layout(
                title=folder,
                autosize=True,
                ...
                scene={
                    "xaxis": {
                        "title": "preformance quality",
                        "nticks": len(config["samples"]["samples"]),
                        "autorange": "reversed",
                    },
                    "zaxis": {"title": "N", "nticks": 10},
                    "yaxis": {
                        "title": "noticing delay",
                        "nticks": len(config["operator"]["noticing_delay"]),
                    },
                    "camera_eye": {"x": 2.2, "y": 2.2, "z": 0.5},
...
def display(folder: str, independent_variable: str) -> None:
    """Display the data in a graph.
    ...
    Returns:
        Shows a graph in a web browser.
    """
    # Load data from a TSV file within the specified folder
    data_frame = pd.read_csv(f"results/{folder}/summary.tsv", sep="\t", index_col=False)
    print(data_frame)

    # Define different dash styles in an array. Extend or adjust styles as needed.
    dash_styles = ["solid", "dash", "dot", "dashdot"]

    # Create a line plot with different dash styles for each line based on the independent variable
    fig = px.line(
        data_frame,
        x="pq",
        y="average",
        color=independent_variable,
        error_y="err",
        color_discrete_sequence=px.colors.qualitative.G10,
        line_dash=independent_variable,
    )

    # Map your independent variable to different dash styles
    # This assumes you have as many styles as categories in your independent variable
    for trace, dash_style in zip(fig.data, dash_styles):
        trace.line.dash = dash_style

    # Update plot layout and settings
    fig.update_traces(showlegend=True, error_y_color="red")
    fig.update_layout(
        title=folder,
        autosize=True,
        margin=dict(l=65, r=50, b=65, t=90),
        xaxis_title="Performance Quality",
        yaxis_title="Noticing Delay",
    )
